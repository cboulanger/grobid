...
HPC clusters typically cannot accept inbound connections. The WebSocket tunnel lets the container **dial out** to a relay server you control, which then proxies your HTTP requests (including live SSE streams) through that connection.

```text
[Your laptop / CI]         [Relay server — internet-accessible]         [HPC job node]

  HTTP client ──────────► relay_server (port 8080) ◄── WS tunnel ── trainer_service.py
  curl, browser, scripts      proxies requests inward                  (port 8072, local)
```

The relay server is a separate, standalone component: **[github.com/cboulanger/grobid-relay](https://github.com/cboulanger/grobid-relay)**. Its README covers installation, systemd service setup, nginx reverse-proxy configuration, fail2ban integration, and full GROBID integration instructions including Apptainer/Slurm job scripts.

To enable the tunnel, pass `--relay` when starting the trainer service:
...
All trainer-service endpoints are then accessible through the relay URL, including SSE streams.
